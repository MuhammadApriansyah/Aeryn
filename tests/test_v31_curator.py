"""Test V31.4 — MemoryCurator: arsip strategi basi, prune episode, dedup skill."""
import json
import os
import sys
import time
...
from aeryn_core.memory_curator import MemoryCurator
...
def test_curate_strategies_arsip_basi(tmp_path):
    ...
    now = time.time()
    refs = [
        {"ts": now - 60 * 86400, "ok": True, "findings": [],
         "recommendations": [], "strategy": "GOAL_SAM: lama"},
        {"ts": now - 3600, "ok": True, "findings": [],
    ...
    kept = [json.loads(l) for l in
            open(str(db / "reflections" / "reflections.jsonl"))]
    assert len(kept) == 1  # yang baru tetap
...
def test_episode_lama_terpangkas_dengan_atom(tmp_path):
    ...
    now = time.time()
    _jl(str(db / "episodes" / "episodes.jsonl"), [
        {"ts": now - 200 * 86400, "ok": True},     # basi + sukses → prune
        {"ts": now - 200 * 86400, "ok": False},    # basi tapi gagal → keep (masih pembelajaran)
        {"ts": now - 3600, "ok": True},            # segar → keep
    ])
    _jl(str(db / "atoms" / "atoms.jsonl"), [{"ts": now, "x": 1}])
    c = MemoryCurator(db_dir=str(db), archive_dir=str(tmp_path / "arch"))
    res = c.curate_episodes(now=now)
    assert res["pruned"] == 1
    left = [json.loads(l) for l in
            open(str(db / "episodes" / "episodes.jsonl"))]
    assert len(left) == 2
    # data terarsip, tidak hilang
    arch = [json.loads(l) for l in
            open(str(tmp_path / "arch" / "episodes_archived.jsonl"))]
    assert len(arch) == 1
...
def test_dedup_skill_fingerprint_sama(tmp_path):
    ...
    left = [json.loads(l) for l in
            open(os.path.join(sf_dir, "skills.jsonl"))]
    fs = [s for s in left if s["fingerprint"] == "fs_read+versi"]
    assert len(fs) == 1 and fs[0]["occurrences"] == 7   # yang terbaik yang tinggal


def test_run_all_gagal_melembut(tmp_path):
    """run_all di DB kosong tidak boleh raise."""
    c = MemoryCurator(db_dir=str(tmp_path / "kosong"),
